...
class LocalExecutor:
    def execute(self, command):
        try:
            ...
            logger.error("local exec err cmd: %s ", command)
            return 1, str(e)
...
EOF''' % (command)
        try:
            ...
            logger.error("local exec err cmd: %s ", exec_cmd)
            return 1, str(e)
...
_allowed_paths = None  # when set, only allow these endpoints

@app.before_request
def _restrict_endpoints_in_local_mode():
...
def local_fast_ip():
    ...
    v2ray_client_json = None

    # 优先：从远端 allow-ip 拉一个“基础 vmess 配置”（历史逻辑）
    if v2ray_order_ip:
        cmd = r'''V2_ORDER_IP=%s bash << 'EOF'  2> /dev/null
curl -fsS "http://${V2_ORDER_IP}:5000/allow-ip" | grep "vmess://" -m 1 | sed 's@^vmess://@@' | base64 -d
EOF''' % (v2ray_order_ip)
        status, output = subprocess.getstatusoutput(cmd)
    ...
    # 回退：直接用本机环境里的 CF 配置拼一个基础模板，避免 V2_ORDER_IP 缺失导致 500
    if v2ray_client_json is None:
        v2ray_client_json = copy.deepcopy(v2ray_client_json := globals().get("v2ray_client_json"))
        v2ray_client_json["host"] = cf_uri
        v2ray_client_json["sni"] = cf_uri
        v2ray_client_json["port"] = cf_ws_port
...
if __name__ == '__main__':
    ...
    if args.local == "True":
        ...
        _allowed_paths = {"/local-fast-ip"}
        start_alive_probe_thread()
        bind_host = os.environ.get("PROXY_MGT_BIND", "127.0.0.1")
        bind_port = int(os.environ.get("PROXY_MGT_PORT", "5000"))
    ...
    if init_iptables():
        ...
        start_alive_probe_thread()
        bind_host = os.environ.get("PROXY_MGT_BIND", "0.0.0.0")
        bind_port = int(os.environ.get("PROXY_MGT_PORT", "5000"))
        app.run(host=bind_host, port=bind_port, threaded=True)
